...
async def monitor_buffer_and_memory():
    ...
    print("=" * 70)
    print()
    print("HYPOTHESIS H3.1: candles_buffer grows unbounded (no rolling window)")
    print("Reality: If buffer grows unbounded → H3.1 VALID")
    print()
    print("VALIDATION:")
    print("  - Run for 5 minutes (300s)")
    print(" - Monitor candles_buffer size every 30s")
    print("  - Monitor memory (RSS) every 30s")
    print("  - Expected: If rolling window = 100, buffer stays <=10")
    ...
    if measurements:
        ...
        if final['max_buffer'] > 100:
            print(f" ✅ H3.1 VALID: No rolling window detected")
            print(f"     Buffer grew to {final['max_buffer']} (expected ≤100 if rolling)")
            h31_valid = True
        else:
            print(f"  ❌ H3.1 INVALID: Rolling window appears to be working")
        ...
        results = {
            ...
            'conclusions': {
                ...
                'projected_hour_mb': final['growth_mb'] * (3600/final['time'])
            }
        }

        with open('TESTS/kilo/test_01_results.json', 'w') as f:
